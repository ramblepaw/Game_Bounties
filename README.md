...
**Updating later:** push code changes to GitHub → the workflow rebuilds and republishes `:latest` automatically → on TrueNAS, restart just the `app` container (Apps → the app → Stop, then Start — not a full NAS reboot). `docker-compose.yaml` sets `pull_policy: always` on `app`, so that restart always fetches whatever is newest at `:latest` instead of silently reusing a stale cached image — no waiting on TrueNAS's own update-check scan, and no background auto-updater. Database migrations run automatically on container start (`docker-entrypoint.sh` runs `prisma migrate deploy`), so schema changes ship with the same redeploy — no manual DB steps.
